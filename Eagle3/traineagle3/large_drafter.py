...
class Model(nn.Module):
    def __init__(self, config, load_head=False, load_emb=True, path=None, eagle3_path=None):
        super().__init__()
        # self.layers = nn.ModuleList(
        #     [LlamaDecoderLayer(config, index=index) for index in range(config.num_hidden_layers)])
        self.midlayer = nn.ModuleList([LlamaDecoderLayeremb(config) for _ in range(num_steps)])
        ...
        for idx in range(self.length):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = torch.utils.checkpoint.checkpoint(
                    create_custom_forward(self.midlayer[step]),
                    inputs_embeds,
                    hidden_states,
                    cache_hidden,
                    attention_mask,
                ...
                layer_outputs = self.midlayer[step](
