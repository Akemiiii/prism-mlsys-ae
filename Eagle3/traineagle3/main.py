...
parser.add_argument('--name', type=str, default='Eagle3')
...
parser.add_argument('--eagle3_path', type=str, default='')
...
set_seed(0)
if args.train_LD:
    logger.info("Training Large Drafter")
    from large_drafter import Model
else:
    logger.info("Training Eagle3")
...
# ShareGPT
dataset_0 = load_dataset('json', data_files=f'{args.datapath}/ShareGPT_V4.3_unfiltered_cleaned_split.json')
...
# UltraChat
dataset_1 = load_dataset(f'{args.datapath}/ultrachat_200k')
...
# OpenThoughts2
dataset_2 = load_dataset(f'{args.datapath}/OpenThoughts2-1M')
...
if args.train_LD:
    model = Model(config, path=args.basepath, load_emb=True, load_head=True, eagle3_path=args.eagle3_path)
else:
...
if global_rank == 0:
    ...
    wandb.init(entity="ciai-llm", project="LD-llama3-8b", name=args.name, mode="offline", config=ds_config)
