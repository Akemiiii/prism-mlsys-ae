...
def write_answers(filename, model_id, questions, answers, num_turns):
    with open(os.path.expanduser(filename), "w") as fout:
        for i in range(len(answers)):
            ans_json = {
                ...
                "choices": {
                    "index": 0,
                    "turns": [answers[i][j] for j in range(num_turns)],
                },
                "tstamp": time.time(),
            }
...
@sgl.function
def answer_mt_bench(s, question_1, question_2=None):
    s += sgl.system(
...
def main(args):
    ...
    arguments = [
        {f"question_{i+1}": q["turns"][i] for i in range(num_turns)} for q in questions
    ]
    ...
    answers = [[s[f"answer_{i+1}"] for i in range(num_turns)] for s in rets]
    ...
    write_answers(answer_file, model_id, questions, answers, args.num_turns)
