...
class LDWorker(TpModelWorker):
    ...
    def __init__(
        ...
        # Load hot token ids
        if self.speculative_algorithm.is_exact_eagle3():
            if server_args.speculative_token_map is not None:
                logger.warning(
                    "Speculative token map specified, but LD3 models already have this. Ignoring the specified token map."
                )
        ...
        if self.speculative_algorithm.is_ld3():
            # LD3 models don't share lm_head
    ...
    def init_attention_backend(self):
        ...
        if self.server_args.attention_backend == "flashinfer":
            ...
            raise ValueError(
                f"LD worker is not supported in attention backend {self.server_args.attention_backend}"
            )
    ...
    def forward_draft_extend(
        ...
        batch.spec_info.draft_step_idx = -1
