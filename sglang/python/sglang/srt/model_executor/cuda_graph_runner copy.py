...
class CudaGraphRunner:
    ...
    def replay_prepare(self, forward_batch: ForwardBatch):
        ...
        if bs != raw_bs:
            # For TARGET_VERIFY mode, each request has num_tokens_per_bs tokens
            self.seq_lens.fill_(self.num_tokens_per_bs)
        ...
        if forward_batch.seq_lens_cpu is not None:
            if bs != raw_bs:
                # For TARGET_VERIFY mode, each request has num_tokens_per_bs tokens
                self.seq_lens_cpu.fill_(self.num_tokens_per_bs)
        ...
        # Attention backend
        # For TARGET_VERIFY mode, each padded request contributes num_tokens_per_bs tokens
        padded_seq_lens_sum = forward_batch.seq_lens_sum + (bs - raw_bs) * self.num_tokens_per_bs
