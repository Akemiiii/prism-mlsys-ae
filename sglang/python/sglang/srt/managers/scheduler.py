...
class Scheduler(
    ...
    def __init__(
        ...
        if self.spec_algorithm.is_eagle():
            if self.spec_algorithm.is_ld():
                from sglang.srt.speculative.ld_worker import LDWorker

                DraftWorkerCls = LDWorker
            else:
                from sglang.srt.speculative.eagle_worker import EAGLEWorker

                DraftWorkerCls = EAGLEWorker

            self.draft_worker = DraftWorkerCls(
                gpu_id=gpu_id,
                tp_rank=tp_rank,
                server_args=server_args,
                nccl_port=port_args.nccl_port,
