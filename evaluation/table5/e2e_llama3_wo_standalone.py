...
def main() -> int:
    ...
    parser.add_argument("--output-csv", default="e2e_llama3.csv")
    parser.add_argument("--server-log", default="logs/e2e_llama3_server.log")
    parser.add_argument("--model-path", default=str(Path(default_model_prefix) / "Meta-Llama-3-8B-Instruct"))
    parser.add_argument(
        "--algorithms",
        nargs="+",
        default=["NONE", "EAGLE", "EAGLE", "PRISM"],
        help="Examples: NONE LD EAGLE",
    )
    parser.add_argument(
        "--draft-model-paths",
        nargs="+",
        default=["-", "EAGLE2-800K-SGLANG/llama3-8b", "HASS-800K-SGLANG/llama3-8b", "PRISM-800K-SGLANG"],
