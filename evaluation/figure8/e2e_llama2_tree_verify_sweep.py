#!/usr/bin/env python3
"""Sweep speculative tree/verify-token configs and evaluate mt_bench throughput."""
...
DEFAULT_DATASETS = ["data/mt_bench/question.jsonl"]
...
def parse_sweep_config(raw: str) -> tuple[int, int, int]:
    parts = [p.strip() for p in raw.split(",")]
    if len(parts) != 3:
        raise ValueError(f"Invalid --sweep-config '{raw}', expected format: steps,topk,draft_tokens")
    try:
        steps, topk, draft_tokens = (int(parts[0]), int(parts[1]), int(parts[2]))
    except ValueError as exc:
        raise ValueError(f"Invalid --sweep-config '{raw}', each field must be int") from exc
    if steps <= 0 or topk <= 0 or draft_tokens <= 0:
        raise ValueError(f"Invalid --sweep-config '{raw}', all fields must be positive")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Sweep speculative tree/verify-token configs and evaluate mt_bench throughput."
    )
    parser.add_argument("--port", type=int, default=30000)
    parser.add_argument("--startup-timeout", type=int, default=180)
    parser.add_argument("--temperature", type=float, default=0.0)
    parser.add_argument("--num-questions", type=int, default=80)
    parser.add_argument("--output-csv", default="e2e_llama2_tree_verify_sweep_avg.csv")
    parser.add_argument("--detail-csv", default="e2e_llama2_tree_verify_sweep_detail.csv")
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=["LD"],
        help="Speculative algorithms to test. Example: LD",
    )
    parser.add_argument(
        "--draft-model-paths",
        nargs="+",
        default=["LD-800k"],
        help="One-to-one with --algorithms.",
    )
    parser.add_argument(
        "--draft-model-prefix",
    ...
    try:
        sweep_configs = [parse_sweep_config(raw) for raw in args.sweep_configs]
    except ValueError as exc:
    ...
    try:
        for num_steps, eagle_topk, num_draft_tokens in sweep_configs:
            for algorithm, draft_path in zip(algorithms, draft_paths):
                ...
                server_log_path = log_dir / f"e2e_llama2_tree_verify_sweep_{run_key}.log"
                ...
                print(
                    f"\n=== Algorithm={algorithm}, steps={num_steps}, "
                    f"topk={eagle_topk}, draft_tokens={num_draft_tokens} ==="
                )
